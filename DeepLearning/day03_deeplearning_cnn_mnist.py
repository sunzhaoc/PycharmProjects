...
def create_model(x):
    ...
    with tf.variable_scope("conv1"):
        ...
        # 定义filter和偏置
        conv1_weight = create_weights(shape=[5, 5, 1, 32])
        conv1_bias = create_weights(shape=[32])
        conv1_x = tf.nn.conv2d(input_x, filter=conv1_weight, strides=[1, 1, 1, 1], padding="SAME") + conv1_bias
        ...
        if FLAGS.is_train == 1:
            ...
            for i in range(100):
                ...
                print("第%d个样本的真实值为：%d，模型预测结果为：%d" % (i+1,
                                                     tf.argmax(sess.run(y_true, feed_dict={x: mnist_x, y_true: mnist_y}), 1),
